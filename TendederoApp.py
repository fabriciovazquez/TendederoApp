...
st.markdown("""
    ...
    .floating-mic-container {
        ...
        font-size: 32px !important; box-shadow: 0px 6px 16px rgba(40, 167, 69, 0.4) !important;
        cursor: pointer; z-index: 999999 !important;
    }
    </style>
...
# ==========================================
# 🎙️ VENTANA EMERGENTE DE VOZ
# ==========================================
if st.session_state.mostrar_modal_voz:
    st.markdown("<div style='background: #E2F0D9; padding: 20px; border-radius: 12px; border: 2px solid #28A745; margin-bottom: 25px;'><b>🎙️ ASISTENTE DE VOZ ACTIVADO</b></div>", unsafe_allow_html=True)
    comando_voz = st.text_input("Dictado:", key="input_dictado_actual")
    if comando_voz:
        cmd = comando_voz.lower()
        if "segundo" in cmd: st.session_state.saldo_segundo += 5.0
        elif "maría" in cmd: st.session_state.saldo_maria += 3.0
...
if st.button("📲 Avisar", key="ws_segundo"):
...
if st.button("📲 Avisar", key="ws_maria"):
    with st.spinner("Enviando..."):
        time.sleep(1)
    st.toast("✅ Mensaje enviado")
st.markdown('</div>', unsafe_allow_html=True)
...
# 🎙️ ACTIVADOR (Sustitución de lógica)
